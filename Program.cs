...
{
    TotalHits = reportService.GetTotalHitCount(logs),
    AverageRtt = reportService.GetAverageResponseTime(logs),
    StatusCodes = reportService.GetStatusCodeReport(logs),
    TopSlowApis = reportService.GetTopSlowApis(logs),
    TopRequestedApis = reportService.GetTopRequestedApis(logs),
    TopErrorApis = reportService.GetTopErrorApis(logs),
    MethodDistribution = reportService.GetMethodDistribution(logs),
    HourlyTraffic = reportService.GetHourlyTrafficReport(logs),
    SlowRequests = reportService.GetSlowRequests(logs)
};

var excelGenerator = new ExcelReportGenerator();

excelGenerator.GenerateReport(report, "LogReport.xlsx");